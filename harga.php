           ...
           <!-- Fungsi Menampilkan Data List Harga dari Database -->					
					...
					<?php
						 if(isset($_GET['hal']) == 'hapus'){
							$id_lapangan = $_GET['kd'];
							$cek = mysqli_query($koneksi, "SELECT * FROM tb_lapangan WHERE id_lapangan='$id_lapangan'");
							if(mysqli_num_rows($cek) == 0){
								echo '<div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> Data tidak ditemukan.</div>';
							}else{
								$delete = mysqli_query($koneksi, "DELETE FROM tb_lapangan WHERE id_lapangan='$id_lapangan'");
								if($delete){
