    ...
    <div class="content-wrapper">
        <div class="container">
            <div class="row">
				...
				<a class="btn btn-success" href="cetak_transaksi_member.php"><i class="fa fa dekstop "></i> Cetak Transaksi</a></div>
                    <h4 class="page-head-line">Menu Transaksi Member</h4>
					
					<?php
             if(isset($_GET['hal']) == 'hapus'){
				$id_transaksi = $_GET['kd'];
				$cek = mysqli_query($koneksi, "SELECT * FROM tb_pesan_member WHERE id_pesan_member='$id_pesan_member'");
				if(mysqli_num_rows($cek) == 0){
					echo '<div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> Data tidak ditemukan.</div>';
				}else{
					$delete = mysqli_query($koneksi, "DELETE FROM tb_pesan_member WHERE id_pesan_member='$id_pesan_member'");
					if($delete){
			...
			<!-- Small boxes (Stat box) -->
              ...
              <div class="col-lg-4">
              <form action='transaksi_member.php' method="POST">
           ...
           <input type='submit' value='Cari Data' class="btn btn-sm btn-primary" /> 
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="alert alert-success">
                        Berikut Record Transaksi Pemesanan Member 
						</div>
                </div>
			...
			<!-- Fungsi Menampilkan Data Transaksi dari Database -->					
					<?php
                    $query1="SELECT tb_pesan_member.id_pesan_member, tb_member.id_member, tb_member.harga, tb_pesan_member.status, user.fullname, tb_pesan_member.gambar, tb_pesan_member.tgl
								FROM tb_pesan_member, tb_member, user
								WHERE user.user_id=tb_pesan_member.user_id AND tb_member.id_member=tb_pesan_member.id_member";
	               ...
	               $query1="SELECT tb_pesan_member.id_pesan_member, tb_member.id_member, tb_member.harga, tb_pesan_member.status, user.fullname, tb_pesan_member.gambar, tb_pesan_member.tgl
								FROM tb_pesan_member, tb_member, user
								WHERE user.user_id=tb_pesan_member.user_id AND tb_member.id_member=tb_pesan_member.id_member
								AND tb_pesan_member.tgl like '%$qcari%'";
                    }
                ...
                <table class="table table-hover table-bordered" id="sampleTable">
                  <thead>
					...
					<tbody>
						...
						<td><center></center></td>
						<?php } ?>
						<td><center><div id="thanks"><a class="btn btn-sm btn-info" data-placement="bottom" data-toggle="tooltip" title="Approve" href="konfirmasi_member.php?hal=konfirmasi&kd=<?php echo $data['id_pesan_member'];?>"><span class="">Approve</span></a>  
